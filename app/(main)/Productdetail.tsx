...
type TabId = "overview" | "variants" | "specifications" | "delivery" | "return";
...
const INITIAL_VARIANTS: Variant[] = [
  { id: "1", color: "Blue",   colorHex: "#3B82F6", size: "Standard", sku: "PEN-BLST-7862", stock: 15, mrp: 2190, discount: 25, sellingPriceExGst: 1642.50, gstPercent: 5,  gstAmount: 82.13,  sellingPriceWithGst: 1724.63, commissionPercent: 15, commissionAmount: 258.69, intraCityDelivery: 175, metroMetroDelivery: 205, totalIntraCity: 2158.32, totalMetroMetro: 2188.32 },
  { id: "2", color: "Green",  colorHex: "#22C55E", size: "Standard", sku: "PEN-GRST-2606", stock: 15, mrp: 2190, discount: 25, sellingPriceExGst: 1642.50, gstPercent: 5,  gstAmount: 82.13,  sellingPriceWithGst: 1724.63, commissionPercent: 15, commissionAmount: 258.69, intraCityDelivery: 175, metroMetroDelivery: 205, totalIntraCity: 2158.32, totalMetroMetro: 2188.32 },
  { id: "3", color: "Red",    colorHex: "#EF4444", size: "Standard", sku: "PEN-REST-2358", stock: 15, mrp: 2190, discount: 25, sellingPriceExGst: 1642.50, gstPercent: 5,  gstAmount: 82.13,  sellingPriceWithGst: 1724.63, commissionPercent: 15, commissionAmount: 258.69, intraCityDelivery: 175, metroMetroDelivery: 205, totalIntraCity: 2158.32, totalMetroMetro: 2188.32 },
  { id: "4", color: "Yellow", colorHex: "#F59E0B", size: "Standard", sku: "PEN-YEST-4683", stock: 15, mrp: 2190, discount: 25, sellingPriceExGst: 1642.50, gstPercent: 5,  gstAmount: 82.13,  sellingPriceWithGst: 1724.63, commissionPercent: 15, commissionAmount: 258.69, intraCityDelivery: 175, metroMetroDelivery: 205, totalIntraCity: 2158.32, totalMetroMetro: 2188.32 },
];
...
// ── CHANGE 2: "Specs" label → "Specifications" ──
const TABS: { id: TabId; label: string; icon: string }[] = [
  { id: "overview",        label: "Overview",       icon: "information-outline"    },
  { id: "variants",        label: "Variants",       icon: "tune-variant"           },
  { id: "specifications",  label: "Specifications", icon: "clipboard-list-outline" }, // ← changed
...
const SectionCard: React.FC<{ children: React.ReactNode; last?: boolean }> = ({ children, last = false }) => (
  <View style={[sub.card, last && { marginBottom: 0 }]}>{children}</View>
);
...
const AddVariantModal: React.FC<{ visible: boolean; onClose: () => void; onAdd: (v: Variant) => void }> = ({ visible, onClose, onAdd }) => {
  ...
  return (
    <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
      <View style={av.root}>
        <TouchableOpacity style={av.overlay} activeOpacity={1} onPress={onClose} />
        <View style={av.sheet}>
          <View style={av.drag} />
...
// ─── VARIANTS GRID VIEW ───────────────────────────────────────
const VariantsGrid: React.FC<{ variants: Variant[]; onDelete: (id: string) => void }> = ({ variants, onDelete }) => (
  <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 10 }}>
    {variants.map(v => (
      <View key={v.id} style={vr.gridCard}>
        {/* Card Header */}
        ...
        {/* Details */}
        <View style={vr.gridRow}>
          <Text style={vr.gridRowLabel}>Stock</Text>
          <Text style={vr.gridRowVal}>{v.stock} units</Text>
        </View>
        <View style={vr.gridRow}>
          <Text style={vr.gridRowLabel}>GST</Text>
          <Text style={[vr.gridRowVal, { color: C.orange }]}>₹{v.gstAmount.toFixed(2)} ({v.gstPercent}%)</Text>
        </View>
        <View style={vr.gridRow}>
          <Text style={vr.gridRowLabel}>Commission</Text>
          <Text style={[vr.gridRowVal, { color: C.red }]}>₹{v.commissionAmount.toFixed(2)}</Text>
        </View>
...
// ─── VARIANTS LIST VIEW ───────────────────────────────────────
const VariantsList: React.FC<{ variants: Variant[]; onDelete: (id: string) => void }> = ({ variants, onDelete }) => (
  <View style={{ gap: 8 }}>
    {variants.map(v => (
      <View key={v.id} style={vr.listCard}>
        ...
        <View style={vr.listPriceRow}>
          <View style={vr.listPriceTile}>
            <Text style={vr.listPriceLabel}>MRP</Text>
            <Text style={[vr.listPriceVal, { color: C.red }]}>₹{v.mrp.toFixed(2)}</Text>
          </View>
          <View style={vr.listPriceTile}>
            <Text style={vr.listPriceLabel}>Excl. GST</Text>
            <Text style={vr.listPriceVal}>₹{v.sellingPriceExGst.toFixed(2)}</Text>
          </View>
          <View style={vr.listPriceTile}>
            <Text style={vr.listPriceLabel}>GST {v.gstPercent}%</Text>
            <Text style={[vr.listPriceVal, { color: C.orange }]}>+₹{v.gstAmount.toFixed(2)}</Text>
          </View>
          <View style={vr.listPriceTile}>
            <Text style={vr.listPriceLabel}>With GST</Text>
            <Text style={[vr.listPriceVal, { color: C.blue, fontFamily: "Outfit_700Bold" }]}>₹{v.sellingPriceWithGst.toFixed(2)}</Text>
          </View>
          <View style={[vr.listPriceTile, { backgroundColor: "#F0FDF4" }]}>
            <Text style={vr.listPriceLabel}>Intra-City</Text>
            <Text style={[vr.listPriceVal, { color: C.green, fontFamily: "Outfit_700Bold" }]}>₹{v.totalIntraCity.toFixed(2)}</Text>
          </View>
          <View style={[vr.listPriceTile, { backgroundColor: "#FFFBEB" }]}>
            <Text style={vr.listPriceLabel}>Metro</Text>
            <Text style={[vr.listPriceVal, { color: C.yellow, fontFamily: "Outfit_700Bold" }]}>₹{v.totalMetroMetro.toFixed(2)}</Text>
          </View>
        </View>
      </View>
    ))}
...
// ─── VARIANTS TABLE VIEW ──────────────────────────────────────
const VariantsTable: React.FC<{ variants: Variant[]; onDelete: (id: string) => void }> = ({ variants, onDelete }) => (
  <ScrollView horizontal showsHorizontalScrollIndicator style={{ marginHorizontal: -14 }}>
    <View style={{ minWidth: SW + 300 }}>
      <View style={vr.tableHead}>
        {["Color","Size","SKU","Stock","MRP\n(Excl. GST)","Discount\n(%)","Selling Price\n(Excl. GST)","GST\n(%)","Selling Price\n(With GST)","Commission\n(% of SP w/ GST)","Intra-City\nDelivery","Metro-Metro\nDelivery","Total\n(Intra-City)","Total\n(Metro-Metro)","Action"].map((col, i) => (
          <View key={i} style={[vr.headCell, i === 0 && { width: 80 }, i === 1 && { width: 80 }, i === 2 && { width: 110 }, i === 3 && { width: 70 }, (i >= 4 && i <= 13) && { width: 100 }, i === 14 && { width: 60 }]}>
      ...
      {variants.map((v, idx) => (
        <View key={v.id} style={[vr.tableRow, { backgroundColor: idx % 2 === 0 ? C.white : "#FAFBFF" }]}>
          ...
          <View style={[vr.cell, { width: 100 }]}>
            <Text style={[vr.cellTxt, { color: C.orange }]}>+ ₹{v.gstAmount.toFixed(2)}</Text>
            <Text style={vr.cellSub}>({v.gstPercent}%)</Text>
          </View>
          ...
          <View style={[vr.cell, { width: 100 }]}>
            <Text style={[vr.cellTxt, { color: C.red }]}>+ ₹{v.commissionAmount.toFixed(2)}</Text>
            <Text style={vr.cellSub}>({v.commissionPercent}%)</Text>
          </View>
          ...
          <View style={[vr.cell, { width: 60, justifyContent: "center" }]}>
            <TouchableOpacity
              style={vr.deleteBtn}
              onPress={() => Alert.alert("Delete Variant", `Remove ${v.color} - ${v.size}?`, [
                { text: "Cancel", style: "cancel" },
            ...
            >
              <MaterialCommunityIcons name="trash-can-outline" size={16} color={C.white} />
            </TouchableOpacity>
          </View>
        </View>
      ))}
...
// ─── VARIANTS TAB ─────────────────────────────────────────────
const VariantsTab: React.FC<{ variants: Variant[]; onAdd: () => void; onDelete: (id: string) => void }> = ({ variants, onAdd, onDelete }) => {
  // ── CHANGE 1: default view is now "list" instead of "grid" ──
  ...
  return (
    <>
      ...
      {variants.length === 0 ? (
        ...
        <VariantsGrid variants={variants} onDelete={onDelete} />
      ) : viewMode === "list" ? (
        <VariantsList variants={variants} onDelete={onDelete} />
      ) : (
        <VariantsTable variants={variants} onDelete={onDelete} />
      )}
...
const vr = StyleSheet.create({
  ...
  summary:         { flexDirection: "row", backgroundColor: C.white, borderRadius: 14, borderWidth: 1, borderColor: C.border, padding: 14, marginTop: 12, justifyContent: "space-around" },
...
const SpecsTab: React.FC<{ p: Product }> = ({ p }) => (
  <>
    ...
    <SectionCard last>
      ...
      <InfoRow label="Box Dimensions"     value={p.packaging.boxDimensions} />
      <InfoRow label="Gross Weight"       value={p.packaging.grossWeight}   />
      <InfoRow label="Packaging Type"     value={p.packaging.packagingType} />
      <InfoRow label="Fragile Item"       value={p.packaging.fragile ? "Yes" : "No"} valueColor={p.packaging.fragile ? C.red : C.green} />
      <InfoRow label="Product Weight"     value={p.weight}                  />
      <InfoRow label="Product Dimensions" value={p.dimensions}              />
    </SectionCard>
  </>
);
...
const ProductDetailScreen: React.FC = () => {
  ...
  const uniqueColors = variants.filter((v, i, arr) => arr.findIndex(x => x.colorHex === v.colorHex) === i);
    ...
    <SafeAreaView style={s.root}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 110 }}>
                ...
                {uniqueColors.map(v => (
                  <View key={v.id} style={s.swatchWrap}>
                    <View style={[
                      s.swatch,
                      { backgroundColor: v.colorHex },
                      v.color === "White" && { borderWidth: 1.5, borderColor: C.border },
                    ]} />
        ...
        <View style={s.tabContent}>
          ...
          {activeTab === "variants"       && <VariantsTab variants={variants} onAdd={() => setShowAddVariant(true)} onDelete={handleDeleteVariant} />}
      ...
      <View style={s.bottomBar}>
        ...
        <TouchableOpacity style={s.editAction} onPress={() => router.push("/(main)/Editproduct" as any)} activeOpacity={0.85}>
          ...
          <Text style={s.editActionText}>Edit Product</Text>
        </TouchableOpacity>
      </View>
...
const s = StyleSheet.create({
  ...
  colorsSection: { marginTop: 12, marginBottom: 14, paddingTop: 12, borderTopWidth: 1, borderTopColor: "#F3F4F6" },
  ...
  bottomBar:        { position: "absolute", bottom: 0, left: 0, right: 0, backgroundColor: C.white, borderTopWidth: 1, borderTopColor: C.border, flexDirection: "row", gap: 8, padding: 12, paddingBottom: Platform.OS === "ios" ? 28 : 14, shadowColor: "#1E2B6B", shadowOffset: { width: 0, height: -4 }, shadowOpacity: 0.10, shadowRadius: 12, elevation: 12 },
  backAction:       { flex: 1, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 5, borderWidth: 1.5, borderColor: C.navy, borderRadius: 13, paddingVertical: 12 },
  backActionText:   { fontFamily: "Outfit_700Bold", fontSize: 13, color: C.navy },
  variantAction:    { flex: 1.5, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 5, backgroundColor: C.orange, borderRadius: 13, paddingVertical: 12 },
  variantActionText:{ fontFamily: "Outfit_700Bold", fontSize: 13, color: C.white },
  editAction:       { flex: 1.5, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 6, backgroundColor: C.navy, borderRadius: 13, paddingVertical: 12 },
  editActionText:   { fontFamily: "Outfit_700Bold", fontSize: 13, color: C.white },
});
