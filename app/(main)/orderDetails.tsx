...
const ORDER_STATUS_CONFIG: Record<string, { bg: string; color: string }> = {
  Pending: { bg: C.bluePale, color: C.blue },
  Processing: { bg: C.purplePale, color: C.purple },
  Shipped: { bg: C.bluePale, color: C.blue },
  Delivered: { bg: C.greenPale, color: C.green },
  Cancelled: { bg: C.orangePale, color: C.orange },
  Failed: { bg: C.redPale, color: C.red },
  Returned: { bg: C.redPale, color: C.red },
  Refunded: { bg: C.orangePale, color: C.orange },
};

function orderStatusStyle(label: string) {
  return ORDER_STATUS_CONFIG[label] ?? { bg: C.bg, color: C.textMid };
}
...
const WebLayout: React.FC<{ order: OrderDetail; onOpenLabel: () => void }> = ({ order, onOpenLabel }) => {
  ...
  const orderStatusCfg = orderStatusStyle(order.status);

  return (
    <View style={{ flex: 1, width: "100%", minWidth: 0, backgroundColor: C.bg }}>

      {/* ── Navy hero header ─────────────────────────────────── */}
      <View style={wl.navyHero}>
        <View style={wl.navyHeroInner}>
          <View style={wl.navyHeroTopRow}>
            <TouchableOpacity
              style={wl.navyBackBtn}
              onPress={() => router.push("/(main)/Ordersscreen")}
              activeOpacity={0.8}
            >
              <Ionicons name="arrow-back" size={18} color={C.white} />
            </TouchableOpacity>
            <Text style={wl.navyHeroTitle}>Order Details</Text>
          </View>
        </View>
      </View>

      {/* ── Floating summary card (overlaps navy hero) ──────────── */}
      <View style={wl.floatingCardWrap}>
        <View style={wl.floatingCard}>
          <View style={wl.floatingCardTopRow}>
            <View style={{ flexDirection: "row", alignItems: "center", gap: 8, flexShrink: 1 }}>
              <Text style={wl.floatingOrderId} numberOfLines={1}>{order.id}</Text>
              <TouchableOpacity onPress={() => { Clipboard.setString(order.id); Alert.alert("Copied", "Order ID copied."); }} hitSlop={{ top: 6, bottom: 6, left: 6, right: 6 }}>
                <Ionicons name="copy-outline" size={14} color={C.textLight} />
              </TouchableOpacity>
              <View style={[wc.pill, { backgroundColor: orderStatusCfg.bg }]}>
                <Text style={[wc.pillText, { color: orderStatusCfg.color }]}>{order.status}</Text>
              </View>
            </View>
            <Text style={wl.floatingDate} numberOfLines={1}>{order.date}</Text>
          </View>
      ...
      <ScrollView
        ...
        contentContainerStyle={[
          wl.content,
          isDesktop && { flexDirection: "row", alignItems: "flex-start", alignSelf: "center", width: "100%", paddingHorizontal: 0 },
...
const MobileLayout: React.FC<{
  ...
  const isWebMobile = Platform.OS === "web" && isMobile; // web narrow only — native mobile app keeps the original, untouched layout
  ...
  return (
    <ScreenRoot style={[styles.safeArea, Platform.OS === "web" && styles.safeAreaWeb]}>
      ...
      <View style={[styles.blueSection, isWebMobile && styles.blueSectionWeb]}>
        ...
        <View style={[styles.floatingTrackerCard, isWebMobile && styles.floatingTrackerCardWeb, isWebMobile && order.extraNote && { marginBottom: -8 }]}>
          <View style={[styles.trackerTopRow, isWebMobile && styles.trackerTopRowWeb]}>
          ...
          {order.extraNote && (
            <View style={[styles.extraNoteRow, isWebMobile && styles.extraNoteRowWeb]}>
      ...
      >
        ...
        <View style={[styles.card, isWebMobile && styles.cardWeb]}>
          <View style={[styles.orderSummaryGrid, isWebMobile && styles.orderSummaryGridWeb]}>
            <View style={isWebMobile ? styles.orderSummaryBlockWeb : styles.orderSummaryLeft}>
              <Text style={styles.labelSmall}>Order ID</Text>
              <View style={[styles.orderIdRow, isWebMobile && styles.orderIdRowWeb]}>
                <Text style={[styles.orderId, isWebMobile && styles.orderIdWeb]} numberOfLines={1} ellipsizeMode="middle">{order.id}</Text>
              </View>
              <View style={[styles.metaRow, isWebMobile && styles.metaRowWeb]}>
            ...
            <View style={isWebMobile ? styles.orderSummaryBlockWeb : styles.orderSummaryRight}>
              ...
              <TouchableOpacity style={styles.amountRow} onPress={() => setAmountExpanded(!amountExpanded)} activeOpacity={0.75}>
              ...
              {amountExpanded && (
                <View style={[styles.amountBreakdown, isWebMobile && styles.amountBreakdownWeb]}>
        ...
        <View style={[styles.card, isWebMobile && styles.cardWeb]}>
          ...
          <View style={[styles.paymentGrid, isWebMobile && styles.paymentGridWeb]}>
            <View style={[styles.paymentCol, isWebMobile && styles.paymentColWeb]}>
              ...
              <Text
                style={[styles.payFieldValue, isWebMobile && styles.payFieldValueWeb]}
                numberOfLines={isWebMobile ? 2 : 1}
                ellipsizeMode={isWebMobile ? "tail" : "middle"}
                selectable={isWebMobile}
              >{order.payment.transactionId}</Text>
...
const wl = StyleSheet.create({
  navyHero: { backgroundColor: C.navy, borderTopLeftRadius: 20, borderTopRightRadius: 20, borderBottomLeftRadius: 20, borderBottomRightRadius: 20, paddingTop: 20, paddingBottom: 44, shadowColor: "#000", shadowOpacity: 0.12, shadowOffset: { width: 0, height: 2 }, shadowRadius: 8, elevation: 4 },
  navyHeroInner: { alignSelf: "center", width: "100%", paddingHorizontal: 24 },
  navyHeroTopRow: { flexDirection: "row", alignItems: "center", gap: 14 },
  navyBackBtn: { alignItems: "center", justifyContent: "center", width: 34, height: 34, backgroundColor: "rgba(255,255,255,0.14)", borderRadius: 17 },
  navyHeroTitle: { color: C.white, fontSize: 19, fontWeight: "800" },
  floatingCardWrap: { alignSelf: "center", width: "100%", paddingHorizontal: 24, marginTop: -32, zIndex: 10 },
  floatingCard: { backgroundColor: C.white, borderRadius: 18, padding: 20, shadowColor: "#000", shadowOpacity: 0.10, shadowOffset: { width: 0, height: 4 }, shadowRadius: 16, elevation: 10 },
  floatingCardTopRow: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 10, marginBottom: 16 },
  floatingOrderId: { fontSize: 16, fontWeight: "800", color: C.textDark },
  floatingDate: { fontSize: 12, color: C.textLight, fontWeight: "600" },
  floatingNoteRow: { flexDirection: "row", alignItems: "center", backgroundColor: C.bg, borderRadius: 8, paddingVertical: 6, paddingHorizontal: 9, marginBottom: 14 },
  floatingNoteText: { fontSize: 11, color: C.textMid, flex: 1, lineHeight: 16 },
  ...
  content: { paddingVertical: 24, paddingTop: 32, paddingHorizontal: Platform.OS === "web" ? 0 : 20, gap: 0, paddingBottom: 40 },
...
const styles = StyleSheet.create({
  ...
  safeAreaWeb: { width: "100%", minWidth: 0, borderRadius: 12, overflow: "hidden" },
  blueSection: { backgroundColor: C.navy, paddingBottom: 8, borderBottomLeftRadius: 16, borderBottomRightRadius: 16 },
  blueSectionWeb: { paddingBottom: 6, borderBottomLeftRadius: 12, borderBottomRightRadius: 12, borderTopLeftRadius: 12, borderTopRightRadius: 12 },
  header: { flexDirection: "row", alignItems: "center", paddingHorizontal: 18, paddingVertical: 10, marginTop: 26 },
  headerWeb: { marginTop: 8, paddingHorizontal: 12, paddingVertical: 10 },
  headerWebMobile: { marginTop: 0, paddingHorizontal: 0, paddingVertical: 4 },
  backBtn: { marginRight: 12, padding: 2 },
  headerTitle: { flex: 1, color: C.white, fontSize: 20, fontWeight: "800" },
  headerTitleWeb: { fontSize: 18, fontWeight: "700" },
  headerActions: { flexDirection: "row", gap: 8 },
  headerIconBtn: { padding: 6 },
  floatingTrackerCard: { marginHorizontal: 14, backgroundColor: C.white, borderRadius: 16, paddingHorizontal: 14, paddingVertical: 8, elevation: 8, shadowColor: "#000", shadowOpacity: 0.12, shadowOffset: { width: 0, height: 4 }, shadowRadius: 14, marginBottom: -24, zIndex: 100 },
  floatingTrackerCardWeb: { marginHorizontal: 0, paddingHorizontal: 10, paddingVertical: 8, borderRadius: 12, marginBottom: -24 },
  trackerTopRow: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginBottom: 6, gap: 8 },
  trackerTopRowWeb: { marginBottom: 6 },
  ...
  stepCircle: { width: 24, height: 24, borderRadius: 12, borderWidth: 1.5, alignItems: "center", justifyContent: "center", marginBottom: 3 },
  stepCircleWeb: { width: 20, height: 20, borderRadius: 10, marginBottom: 2 },
  stepLabel: { fontSize: 9, textAlign: "center", lineHeight: 11 },
  stepLabelWeb: { fontSize: 8, lineHeight: 9, width: "100%" },
  stepConnectorWrap: { flex: 1, alignItems: "center", marginTop: 11, paddingHorizontal: 2 },
  stepConnectorWrapWeb: { flex: 0.6, marginTop: 9, paddingHorizontal: 1, minWidth: 4 },
  stepConnectorLine: { height: 2, width: "100%", borderRadius: 2, backgroundColor: C.border },
  extraNoteRow: { flexDirection: "row", alignItems: "center", marginTop: 6, backgroundColor: C.bg, borderRadius: 8, paddingVertical: 4, paddingHorizontal: 8 },
  extraNoteRowWeb: { marginTop: 6, marginBottom: 4, paddingVertical: 6, paddingHorizontal: 9 },
  extraNoteText: { fontSize: 11, color: C.textMid, flex: 1, lineHeight: 16 },
  scrollView: { flex: 1, backgroundColor: C.bg },
  scrollContent: { padding: 14, paddingTop: 36, gap: 12, paddingBottom: 32 },
  scrollContentWeb: { paddingHorizontal: 0, paddingTop: 36, paddingBottom: 24 },
  scrollContentWebMobile: { paddingTop: 20, gap: 10, paddingBottom: 32, paddingHorizontal: 12 },
  ...
  orderSummaryGridWeb: { flexDirection: "column", gap: 6 },
  orderSummaryLeft: { flex: 1 },
  orderSummaryRight: { flex: 1, alignItems: "stretch" },
  orderSummaryBlockWeb: { width: "100%" },
  ...
  amountBreakdown: { marginTop: 6, gap: 2, paddingBottom: 4 },
  amountBreakdownWeb: { marginTop: 8, gap: 6, paddingBottom: 10 },
  amountBreakdownRow: { flexDirection: "row", justifyContent: "space-between", gap: 16 },
  amountBreakdownLabel: { fontSize: 11, color: C.textLight, lineHeight: 16 },
  amountBreakdownValue: { fontSize: 11, color: C.textMid, fontWeight: "600", lineHeight: 16 },
  sectionHeader: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginBottom: 14 },
  sectionHeaderLeft: { flexDirection: "row", alignItems: "center", gap: 8 },
  sectionIconWrap: { width: 32, height: 32, borderRadius: 8, backgroundColor: C.bluePale, alignItems: "center", justifyContent: "center" },
  ...
  paymentCol: { flex: 1 },
  paymentColWeb: { width: "100%", flex: undefined, marginBottom: 4 },
  payFieldLabel: { fontSize: 11, color: C.textLight, fontWeight: "500", marginBottom: 2 },
  payFieldValue: { fontSize: 13, fontWeight: "600", color: C.textDark, lineHeight: 20 },
  payFieldValueWeb: { fontSize: 12, lineHeight: 17 },
